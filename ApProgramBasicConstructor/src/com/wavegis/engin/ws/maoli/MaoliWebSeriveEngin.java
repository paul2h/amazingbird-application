...
public class MaoliWebSeriveEngin extends TimerEngin{
	...
	public void timerAction(){
		...
		if(stationInfos == null){
			...
			stationInfos = (StationInfo[])getResponseData(actionURL + actionName, createAuthAction(keyResult, actionName), StationInfo[].class);
		}
		showMessage("取得各別測站的測量資料中...");
		
		//List<RainStationData> rainStationDatas = new ArrayList<RainStationData>();
		//List<RawStationData> rawStationDatas = new ArrayList<RawStationData>();
		...
		for(StationInfo stationInfo : stationInfos){
			...
			StationDataInfo[] stationDatas = (StationDataInfo[])getResponseData(actionURL + stActionName, stParam, StationDataInfo[].class);
				...
				if(!dataName.trim().isEmpty()){
					...
					if(data != null){
						if("水位".equals(dataName)){
							/*
							RawStationData rawData = new RawStationData();
							
							rawData.setDatatime(Timestamp.valueOf(data.getData().getTimeStampIso()));
						...
						if("10分鐘累積雨量".equals(dataName)){
							/*
							RainStationData rainData = new RainStationData();
							
							rainData.setDatatime(Timestamp.valueOf(data.getData().getTimeStampIso()));
	...
	private String dataEncryption(KeyResult keyResult, String message){
		...
		SecretKey secretKey = new SecretKeySpec(encryptKey.getBytes(), MAC_NAME);// 輸入參數為資料byte & 指定一個密鑰算法的名稱
		try {
